...
Add an event handler to hover on scatter points and highlight them, i.e. change the edge color and size of the
clicked point. Fly around the 3D scatter using WASD keys and click on points to highlight them.
...
scatter = figure[0, 0].add_scatter(
    ...
    sizes=np.repeat(10, len(data)),  # some big points that are easy to click
    edge_colors=np.zeros((len(data), 4)),  # per-point edge colors that we will change
    edge_width=5,
    cmap="viridis",
...
def highlight_point(ev: pygfx.PointerEvent):
    ...
    if old_props["index"] is not None:
        ...
        if new_index == old_index:
            ...
            return
        scatter.edge_colors[old_index] = old_props["edge_colors"]
        scatter.sizes[old_index] = old_props["size"]

    # store the current property values of this new point
    old_props["index"] = new_index
    old_props["edge_colors"] = scatter.edge_colors[new_index].copy()  # if you do not copy you will just get a view of the array!
