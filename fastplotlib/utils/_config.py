...
class ConfigDescriptor:
    ...
    def __get__(self, instance, cls: type = None):
        ...
        if cls not in self.__classes.keys():
            raise LookupError("Class is not registered")
...
class Config:
    ...
    def register(self, cls):
        ...
        if not self._pending[-1].belongs_to(cls):
            raise TypeError(
                f"{self._pending[-1].cls_qual} is not registered with the global config"
            )

        method_dcs = {p.method.__name__: p.to_dataclass() for p in self._pending}

        self._register(cls, method_dcs)
    ...
    def _register(self, cls, method_dcs: dict[str, object]):
        ...
        dc = make_dataclass(
            cls.__name__,
            fields=[
                (m, object, field(default_factory=mdc)) for m, mdc in method_dcs.items()
            ],
            slots=True,
            frozen=True,
        )
