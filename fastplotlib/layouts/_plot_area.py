...
class PlotArea:
    ...
    def get_nearest_graphics(
        ...
        method: "center" | "edge"
            ...
            Note that the "edge" method uses an axis-aligned bounding box and therefore
            maybe be inaccurate if the Graphic is not axis-aligned, ex. if it is rotated

        subset: np.ndarray[Graphic], optional
        ...
        if subset is not None:
            ...
            if not all(isinstance(g, Graphic) for g in subset):
                raise TypeError("all elements of `subset` must be Graphic objects")
        ...
        match method:
            ...
            case "edge":
                # use bbox
                distances = np.empty(shape=(len(graphics),))
                for i in range(distances.size):
                    bbox = graphics[i].world_object.get_world_bounding_box()

                    distances[i] = np.linalg.norm(
                        np.abs(bbox[:, :len(pos)] - pos).min(axis=0)
                    )

            case _:
                raise ValueError("method must be one of 'center' or 'edge'")
